...
internal readonly struct UnionGenHelper(UnionToGenerate union)
{
    ...
    public string GeneratePartialStruct()
    {
        ...
        unionBuilder.AppendLine("using System;");
        unionBuilder.AppendLine("using System.Threading.Tasks;");
        unionBuilder.AppendLine($"namespace {union.Namespace}");
        unionBuilder.AppendLine("{");
        unionBuilder.AppendLine(prefixNesting);
        unionBuilder.AppendLine($"    {union.Visibility} readonly partial struct {union.Name} : IEquatable<{union.Name}>");
        unionBuilder.AppendLine("    {");
    ...
    private string GenerateSwitch(bool asAsync)
    {
        ...
        var delegateType = asAsync ? "Func" : "Action";
        var secondParameter = asAsync ? ", ValueTask" : string.Empty;
        var awaitPrefix = asAsync ? "await " : string.Empty;
        for (var i = 0; i < union.TypeParameters.Count; i++)
        {
            var type = union.TypeParameters[i];
            var actionName = $"for{type.TitleCaseName}";
            parameters.Append($"{delegateType}<{type.FullName}{secondParameter}> {actionName}");
            ...
            cases.AppendLine(type.IsReferenceType
                                 ? $"case {i}: {awaitPrefix}{actionName}(({type.FullName}){RefValueFieldName}!); break;"
        ...
        var methodSignature = asAsync ? "async ValueTask SwitchAsync" : "void Switch";
    ...
    private string GenerateMatch(bool asAsync)
    {
        var parameters = new StringBuilder();
        var cases = new IndentedStringBuilder(4);
        var secondParameter = asAsync ? "ValueTask<TResult>" : "TResult";
        var awaitPrefix = asAsync ? "await " : string.Empty;
        for (var i = 0; i < union.TypeParameters.Count; i++)
        {
            var type = union.TypeParameters[i];
            var funcName = $"with{type.TitleCaseName}";
            parameters.Append($"Func<{type.FullName}, {secondParameter}> {funcName}");
            if (i < union.TypeParameters.Count - 1)
            {
                parameters.Append(", ");
            }
            ...
            cases.AppendLine(type.IsReferenceType
                                 ? $"{i} => {awaitPrefix}{funcName}(({type.FullName}){RefValueFieldName}!),"
        ...
        var methodSignature = asAsync ? "async ValueTask<TResult> MatchAsync<TResult>" : "TResult Match<TResult>";
    ...
    private string GenerateGetTypeName()
    {
        var func
            = new IndentedStringBuilder(2,
                                        $"public string {TypeLookupFunc}(int index) =>{IndentedStringBuilder.NewLine}");
