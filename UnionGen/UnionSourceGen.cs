...
public sealed class UnionSourceGen : IIncrementalGenerator
{
    ...
    private const string DefaultAlignment = $"{AlignmentEnumName}.Aligned4";
    ...
    private static (string, string[]) CreateAttributeCode(string ns)
    {
        ...
        for (var i = MinTypeParameters; i <= MaxTypeParameters; i++)
        {
            ...
            attributes.AppendLine("[AttributeUsage(AttributeTargets.Struct)]");
            attributes.AppendLine($"public sealed class {AttributeName}<{typeParams}> : Attribute");
